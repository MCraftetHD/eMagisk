...
get_config() {
	if [[ -s $CONFIGFILE ]]; then
		log -p i -t eMagiskCosmogJpv2 "$CONFIGFILE exists and has data. Data will be pulled."
		source $CONFIGFILE
		export rdm_user rdm_password rdm_backendURL discord_webhook timezone autoupdate
	else
		log -p i -t eMagiskCosmogJpv2 "Failed to pull the config file. Make sure $($CONFIGFILE) exists and has the correct data."
	fi
}
...
check_mitmpkg() {
	if [ "$(pm list packages com.sy1vi3.cosmog)" = "package:com.sy1vi3.cosmog" ]; then
		log -p i -t eMagiskCosmogJp "Found Cosmog!"
...
get_deviceName() {
	if { [[ "$MITMPKG" == "com.sy1vi3.cosmog" || "$MITMPKG" == "com.nianticlabs.pokemongo.ares" ]] && [ -f /data/local/tmp/cosmog.json ]; }; then
		mitmDeviceName=$(jq -r '.device_id' /data/local/tmp/cosmog.json)
	else
		log -p i -t eMagiskCosmogJp "Couldn't find the config file"
	fi
}
...
force_restart() {
	...
	if [[ $MITMPKG == com.sy1vi3* ]] || [[ $MITMPKG == "com.nianticlabs.pokemongo.ares" ]]; then
...
webhook() {
	...
	if [[ -z "$discord_webhook" ]]; then
		log -p i -t eMagiskCosmogJp "discord_webhook variable is not set. Cannot send webhook."
		return
	fi
	...
	if ! ping -c 1 -W 1 8.8.8.8 >/dev/null && ! ping -c 1 -W 1 1.1.1.1 >/dev/null; then
		log -p i -t eMagiskCosmogJp "No internet connectivity. Skipping webhook."
		return
	fi
	...
	log -p i -t eMagiskCosmogJp "Sending discord webhook"
	...
	if [[ $http_status -eq 200 ]]; then
	  ...
	  if [[ $first_line = '#!/system/bin/sh' ]] && [[ $last_line = '#ENDOFFILE' ]]; then
		...
		if ! cmp -s updated_script.sh "$script_path"; then
		  ...
		  log -p i -t eMagiskCosmogJp "[AUTOUPDATE] ATVServices.sh was auto updated"
		  ...
		  log -p i -t eMagiskCosmogJp  "[AUTOUPDATE] The downloaded script is identical to the existing script."
		  rm -f updated_script.sh
		fi
	  else
		log -p i -t eMagiskCosmogJp  "[AUTOUPDATE] The downloaded script does not have the expected shebang."
		log -p i -t eMagiskCosmogJp  "[AUTOUPDATE] It had: $first_line"
		webhook "[AUTOUPDATE] The downloaded script does not have the expected shebang."
	  fi
	else
	  log -p i -t eMagiskCosmogJp  "[AUTOUPDATE] Failed to download the updated script. HTTP status code: $http_status"
...
# if [ "$(pm list packages -e com.android.vending)" = "package:com.android.vending" ]; then
	# log -p i -t eMagiskCosmogJp "Disabling Play Store"
...
if [ -n "$timezone" ]; then
	...
	log -p i -t eMagiskCosmogJp "Timezone set to $timezone"
else
	log -p i -t eMagiskCosmogJp "Timezone variable not set. Skipping timezone change."
fi
...
if result=$(check_mitmpkg); then
	(
		...
		if [ "$autoupdate" = "true" ]; then
		  log -p i -t eMagiskCosmogJp "[AUTOUPDATE] Checking for new updates"
		  autoupdate
		else
		  log -p i -t eMagiskCosmogJp "[AUTOUPDATE] Disabled. Skipping"
		fi
		webhook "Booting"
		while :; do  
			if [[ $counter -gt 3 ]];then
				log -p i -t eMagiskCosmogJp "Critical restart threshold of $counter reached. Rebooting device..."
			...
			if [ -n "$rdm_user" ] && [ -n "$rdm_password" ] && [ -n "$rdm_backendURL" ]; then # In case rdm variables are confiugred
				log -p i -t eMagiskCosmogJp "Started rdm health check!"
				...
				if [ "$statusCode" -ne 200 ]; then
					...
					log -p i -t eMagiskCosmogJp "RDM statusCode error: $message"
					continue
				fi
				...
				do
					log -p i -t eMagiskCosmogJp "Found our device! Checking for timestamps..."
					calcTimeDiff=$(( $rdmTimestamp - $lastSeen ))

					if [[ $calcTimeDiff -gt 300 ]]; then
						log -p i -t eMagiskCosmogJp "Last seen at RDM is greater than 5 minutes -> MITM Service will be restarting..."
						force_restart
						led_blue
						counter=$((counter+1))
						log -p i -t eMagiskCosmogJp "Counter is now set at $counter. device will be rebooted if counter reaches 4 failed restarts."
						...
						log -p i -t eMagiskCosmogJp "Our device is live!"
						counter=0
						led_red
					else
						log -p i -t eMagiskCosmogJp "Last seen time is a bit off. Will check again later."
						counter=0
						led_red
					fi
				done
				log -p i -t eMagiskCosmogJp "Scheduling next check in 4 minutes..."
				...
				if [[ $MITMPKG == com.sy1vi3* ]] || [[ $MITMPKG == "com.nianticlabs.pokemongo.ares" ]]; then
					if ! ps -a | grep -vE "grep|worker" | grep "$MITMPKG" ; then
						log -p i -t eMagiskCosmogJp "Process $MITMPKG is not alive, starting it"
						am start -n $MITMPKG/.MainActivity
						counter=$((counter+1))
      					else
	   					log -p i -t eMagiskCosmogJp "Process $MITMPKG is alive. No action required."
     						counter=0
     					fi
	  				continue
					...
					log -p i -t eMagiskCosmogJp "No MITM detected ($MITMPKG?), skipping health check."
					continue
				fi
				...
				if [[ $calcTimeDiff -le 120 ]]; then
					      log -p i -t eMagiskCosmogJp "The log was modified within the last 120 seconds. No action required."
     			      counter=0
					      led_red # turn red when service is up
				else
					      log -p i -t eMagiskCosmogJp "The log wasn't modified within the last 120 seconds. Forcing restart of MITM. ts: $timestamp_epoch, time now: $current_time"
					      force_restart
					      counter=$((counter+1))
...
monitor_and_launch() {
	while true; do
		...
		log -p i -t eMagiskCosmogJp "Cosmog launched. Waiting 20 Minutes..."

		# Check logcat for "IntegritySolver" within the last 2 minutes
		log -p i -t eMagiskCosmogJp "Checking logcat for IntegritySolver."
		if ! logcat -d | grep "IntegritySolver"; then
			log -p e -t eMagiskCosmogJp "IntegritySolver not found in logcat. Restarting device."
			reboot
		else
